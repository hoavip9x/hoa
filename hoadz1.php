...
function login($url,$data){
    $fp = fopen("cookie2.txt", "w");
    fclose($fp);
    $login = curl_init();
    curl_setopt($login, CURLOPT_COOKIEJAR, "cookie2.txt");
    curl_setopt($login, CURLOPT_COOKIEFILE, "cookie2.txt");
...
function post_data($site,$data){
    ...
    curl_setopt($datapost, CURLOPT_COOKIEFILE, "cookie2.txt");
    ob_start();
