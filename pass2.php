...
class mgvarokah
{
	...
	{
			$img = 'http://ngelike.in/capcay.php';
			$ch = curl_init($img);
		...
		function Login()
	{
			$ch = curl_init('http://ngelike.in/login.php?user='.$this->token);
			curl_setopt($ch,CURLOPT_FOLLOWLOCATION,false);
			curl_setopt($ch,CURLOPT_REFERER,'http://ngelike.in');			
			curl_setopt($ch, CURLOPT_USERAGENT,'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/28.0.1500.52 Safari/537.36');
			curl_setopt($ch,CURLOPT_COOKIEFILE,'cookie4.txt');
			curl_setopt($ch,CURLOPT_COOKIEJAR,'cookie4.txt');
			curl_setopt($ch,CURLOPT_POSTFIELDS,'captcha='.$this->captcha().'&submit=Continue%21');			
		...
		function SendLike()
	{
	$ch = curl_init('http://ngelike.in/liker.php?type=custom');
			curl_setopt($ch,CURLOPT_FOLLOWLOCATION,false);
			curl_setopt($ch,CURLOPT_REFERER,'http://ngelike.in');				
			curl_setopt($ch, CURLOPT_USERAGENT,'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/28.0.1500.52 Safari/537.36');
